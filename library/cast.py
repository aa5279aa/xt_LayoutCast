...
def getother_jar(rootdir):
    if os.path.isfile(os.path.join(rootdir, 'local.properties')):
        data = open_as_text(os.path.join(rootdir, 'local.properties'))
        for line in re.findall(r'''otherJarPath=\s*(.+)''', data):
            return line
    return ''
def is_exe(fpath):
...
def __deps_list_gradle(list, project, rootdir):
    data = open_as_text(os.path.join(rootdir, 'local.properties'))
    ideps = []
    for line in re.findall(r'''layoutPath=\s*(.+)''', data):
...
def deps_list(dir,rootdir):
    if is_gradle_project(dir):
        list = []
        __deps_list_gradle(list, dir, rootdir)
        return list
    else:
        list = []
        __deps_list_eclipse(list, dir, rootdir)
        return list
...
def get_aapt(path):
    data = open_as_text(os.path.join(rootdir, 'local.properties'))
    for line in re.findall(r'''aaptPath=\s*(.+)''', data):
        btpath = line
    execname = os.name=='nt' and 'aapt.exe' or 'aapt'
    if is_exe(os.path.join(btpath, execname)):
        minp = os.path.join(btpath, execname)
        return minp
...
if __name__ == "__main__":
    ...
    if srcModified:
        ...
        if vmversion.startswith('1') or vmversion.startswith('2'):
            ...
            otherJarPath = getother_jar(rootdir)
            classpath.append(otherJarPath)
            print('classpath')
            print(classpath)
            for dep in adeps:
                dlib = libdir(dep)
